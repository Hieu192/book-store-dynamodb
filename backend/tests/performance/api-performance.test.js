...
describe('API Performance Tests', () => {
  ...
  describe('Response Time Tests', () => {
    it('GET /api/v1/products should respond within 1000ms', async () => {
    ...
    it('GET /api/v1/product/:id should respond within 500ms', async () => {
      const product = await createTestProduct(testUser._id || testUser.id);
      const startTime = Date.now();

      const res = await request(app)
        .get(`/api/v1/product/${product._id || product.id}`)
        .expect(200);
    ...
    it('POST /api/v1/login should respond within 2000ms', async () => {
      const startTime = Date.now();
    ...
    it('GET /api/v1/me should respond within 500ms', async () => {
      const startTime = Date.now();
  ...
  describe('Database Query Performance', () => {
    beforeAll(async () => {
      // Create 20 test products (reduced for faster testing)
      const promises = Array(20).fill(null).map((_, i) =>
        createTestProduct(testUser._id || testUser.id, {
          name: `Performance Test Product ${timestamp}-${i}`,
          price: 100 + i
        })
      );
      await Promise.all(promises);
    ...
    it('should fetch paginated products efficiently', async () => {
      ...
      console.log(`📊 Paginated Query (limit 20):`);
    ...
    it('should filter products efficiently', async () => {
      ...
      const res = await request(app)
        .get('/api/v1/products?price[gte]=100&price[lte]=110')
        .expect(200);
  ...
  describe('Memory Usage', () => {
    it('should not leak memory on repeated requests', async () => {
      ...
      // Make 30 requests (reduced for faster testing)
      for (let i = 0; i < 30; i++) {
      ...
      console.log(`📊 Memory Usage (30 requests):`);
